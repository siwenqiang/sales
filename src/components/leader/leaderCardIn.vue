    ...
    methods:{
        ...
        getCardInData(){
            ...
            $.ajax({
              ...
              url: "http://192.168.2.124/salestest/team/SysAdminGetOnPunch" ,//url
              ...
              success: function (res) {
                if(res.recode!=400){
                  ...
                  console.log(res)
                  //为了拿到数组中的对象中的数组中的对象的键值
                  for(var i = 0;i<res.data.length;i++){
                      if(res.data[i].punch_info.length!==0){
                          for(var j = 0;j<res.data[i].punch_info.length;j++){
                              _this.tableData.push(res.data[i].punch_info[j])
                          }
                      }
                  }
